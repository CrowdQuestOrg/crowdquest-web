...
import { Input } from "../components/ui/input";
import VirtualGrid from "../components/VirtualGrid";
import { Coins, Grid3X3, MapPin, Plus, Link2, UserPlus, Trash2, Send, Loader2, ShieldCheck } from "lucide-react";
import { useToast } from "../hooks/use-toast";
...
const CreateGamePage = () => {
  ...
  const { createQuest } = useCrowdQuest();
  ...
  // Check allowance whenever address or stakeAmount changes
  useEffect(() => {
    ...
    updateAllowance();
  }, [address, checkAllowance, isPending]);
  ...
  const handleApprove = async () => {
    ...
    setIsPending(true);
    try {
      toast({ title: "Approving Tokens...", description: "Please confirm in MetaMask" });
      await approve(stakeAmount);
      toast({ title: "Tokens Approved!", description: "You can now create your quest." });
    } catch (error) {
  ...
  const handleCreate = async () => {
    ...
    setIsPending(true);
    try {
      toast({ title: "Creating Quest...", description: "Sending transaction to Sepolia" });
      
      // We send the first hint as the 'Name' for now
      const receipt = await createQuest(hints[0], Number(stakeAmount));
      
      if (receipt) {
        toast({ title: "Quest Created Successfully!", description: "Game is now live on-chain." });
        navigate("/hunt");
      }
    } catch (error) {
      console.error(error);
  ...
  // Helper UI functions
  const addHint = () => hints.length < 5 && setHints([...hints, ""]);
  const removeHint = (index: number) => hints.length > 3 && setHints(hints.filter((_, i) => i !== index));
  const updateHint = (index: number, value: string) => {
    const updated = [...hints];
    updated[index] = value;
    setHints(updated);
  ...
  if (!isConnected) {
    return <div className="container py-20 text-center">Please connect your wallet to create a game.</div>;
  }
    ...
    <div className="container py-8 max-w-3xl">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={() => setMode("virtual")}
            className={`flex flex-col items-center gap-2 rounded-xl border p-5 transition-all ${
              mode === "virtual" ? "border-accent bg-accent/10 shadow-md" : "bg-card hover:border-accent/40"
            }`}
          >
            <Grid3X3 className={`h-8 w-8 ${mode === "virtual" ? "text-accent" : "text-muted-foreground"}`} />
            <span className="font-display text-sm font-semibold text-foreground">Play Virtually</span>
          ...
          <button
            onClick={() => setMode("offline")}
            className={`flex flex-col items-center gap-2 rounded-xl border p-5 transition-all ${
              mode === "offline" ? "border-accent bg-accent/10 shadow-md" : "bg-card hover:border-accent/40"
            }`}
          >
            <MapPin className={`h-8 w-8 ${mode === "offline" ? "text-accent" : "text-muted-foreground"}`} />
            <span className="font-display text-sm font-semibold text-foreground">Play Offline</span>
      ...
      {mode && (
        <>
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                disabled={isPending}
              />
              <span className="text-sm text-muted-foreground">CQT</span>
            </div>
            {needsApproval && stakeAmount && (
              <p className="text-xs text-amber-500 mt-2 flex items-center gap-1">
                <ShieldCheck className="h-3 w-3" /> Approval required for {stakeAmount} CQT
              </p>
            )}
          </div>
          ...
          {/* 4. Hints */}
          <div className="mb-8">
            <h2 className="font-display text-sm font-semibold text-foreground mb-3">Progressive Hints</h2>
            <div className="space-y-2">
              {hints.map((hint, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Input
              ...
              {hints.length < 5 && (
                <Button variant="ghost" size="sm" onClick={addHint} disabled={isPending}>
          ...
          {/* Final Action Button */}
          {needsApproval ? (
            <Button variant="hero" size="lg" className="w-full gap-2" onClick={handleApprove} disabled={isPending}>
              {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}
              Approve CQT Staking
            </Button>
          ) : (
            <Button variant="hero" size="lg" className="w-full gap-2" onClick={handleCreate} disabled={isPending}>
