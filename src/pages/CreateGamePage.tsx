...
const CreateGamePage = () => {
  ...
  // const { createVirtualQuest, createRealWorldQuest } = useCrowdQuest();
  const { getAddressByUsername } = useProfile();
  const { createLobby, submitPlayerConfig, activeLobby } = useGameLobby();
  ...
  // 4. Create Lobby instead of direct deployment
  const handleCreateLobby = async () => {
    ...
    if (mode === "virtual" && !selectedCell) {
      toast({ 
        title: "Validation Error", 
        description: "Target coordinates must be locked on the grid.", 
        variant: "destructive" 
      });
      return;
    }
    ...
    try {
      // Create the lobby
      const invitedUsernames = invitedPlayers.map(p => p.name);
      const lobby = await createLobby(mode, stakeAmount, invitedUsernames);
      
      // Submit creator's configuration
      await submitPlayerConfig(lobby.gameId, {
        selectedCell: selectedCell || undefined,
        hints: cleanHints,
        stakeAmount: stakeAmount,
      });
      
      setShowLobbyStatus(true);
      toast({ 
        title: "Lobby Created", 
        description: `Share the invite link with ${invitedPlayers.length} player(s)` 
      });
    } catch (err: any) {
  ...
  return (
    <div className="container py-10 max-w-3xl space-y-12">
      ...
      {mode && (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="pt-6 border-t space-y-4">
            ...
            <p className="text-center text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
              Create lobby and wait for players to configure their treasures
            </p>
          </div>
        </div>
      )}
      ...
      {/* LOBBY STATUS MODAL */}
      {showLobbyStatus && activeLobby && (
        <LobbyStatus 
          lobby={activeLobby} 
          onGameStart={() => navigate("/hunt")}
        />
      )}
    </div>
  );
